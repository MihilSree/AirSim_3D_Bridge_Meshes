...
int main(int argc, char** argv){
  std::ofstream myfile;
  myfile.open("/home/mihil/bridgeInspection/testInfo.txt");
  std::ofstream myfile1;
  myfile1.open("/home/mihil/bridgeInspection/testClusters.txt");
  std::ofstream myfileT;
  myfileT.open("/home/mihil/bridgeInspection/timeVSTrimmedOcc.csv");
  std::ofstream myfileDA; // actual distance
  myfileDA.open("/home/mihil/bridgeInspection/timeVSDistanceActual.csv");
  std::ofstream myfileDE; // expected distance
  myfileDE.open("/home/mihil/bridgeInspection/timeVSDistanceExpected.csv");
  std::ofstream myfileCompTime;
  myfileCompTime.open("/home/mihil/bridgeInspection/timeVSComputationalTime.csv");
  std::ofstream myfileDR; // real distance
  myfileDR.open("/home/mihil/bridgeInspection/timeVSDistanceReal.csv");
...
//taking off
  ...
  cout << "Press Enter to move to (0,0,1, 1) with 1 m/s velocity" << endl; cin.get();
  client.moveToPositionAsync(position.x() + 0, position.y() + 0, position.z() - 1, 1)->waitOnLastTask();

  cout << "Press Enter to move to (0,-1,3, 1) with 1 m/s velocity" << endl; cin.get();
  client.enableApiControl(true);
  client.armDisarm(true);
  client.moveToPositionAsync(position.x() + 0, position.y() + -1, position.z() - 3, 1)->waitOnLastTask();

  cout << "Press Enter to move to (0,-1,6, 1) with 1 m/s velocity" << endl; cin.get();
  client.enableApiControl(true);
  client.armDisarm(true);
  client.moveToPositionAsync(position.x() + 0, position.y() + -1, position.z() - 6, 1)->waitOnLastTask();
...
// executes tour based on GTSP output
    if(structureCovered == 1){ // going back to origin because no new viewpoints
      ROS_INFO("Moving to point: (%f,%f,%f)", 0,0,0);
      ROS_INFO("Sending goal");

      cout << "Press Enter to land" << endl; cin.get();
      client.enableApiControl(true);
      client.armDisarm(true);
      client.moveToPositionAsync(position.x() + 0, position.y() + 0, position.z() - 0, 1)->waitOnLastTask();
